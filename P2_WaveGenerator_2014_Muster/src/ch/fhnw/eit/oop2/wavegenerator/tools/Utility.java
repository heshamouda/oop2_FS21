...
public class Utility {
	...
	public static ImageIcon loadResourceIcon(String strBild) {
		ImageIcon icon = new ImageIcon(Utility.class.getResource("resources/"
				+ strBild));
		return icon;
	}
	...
	public static Image loadScaledResourceImage(String strBild, int width,
			int height, int hints) {
		MediaTracker tracker = new MediaTracker(p);
		Image img = (new ImageIcon(Utility.class.getResource("resources/"
				+ strBild))).getImage().getScaledInstance(width, height, hints);
		tracker.addImage(img, 0);
		try {
			tracker.waitForID(0);
		} catch (InterruptedException ex) {
