...
public class PlotAxes extends AbstractAxes implements MouseListener, MouseMotionListener {
	...
	public void paintComponent(Graphics z) {
		...
		if (bgImage != null) {
			g.setClip(leftInset, topInset, innerWidth, innerHeight);
			double min = Math.min((double) (this.getWidth()) / bgImage.getWidth(this),
					(double) (this.getHeight()) / bgImage.getHeight(this));

			int bildBreite = (int) (bgImage.getWidth(this) * min);
			int bildHoehe = (int) (bgImage.getHeight(this) * min);
			int randLinks = (this.getWidth() - bildBreite) / 2;
			int randOben = (this.getHeight() - bildHoehe) / 2;

			g.drawImage(bgImage, randLinks, randOben, bildBreite, bildHoehe, this);

//			g.drawImage(img, leftInset, topInset, this);
		}
			...
			if (this.axesMode == LIN_LIN) {
				for (int i = (int) Math.ceil(xMin / majorXTickSpacing); i <= (int) Math.floor(xMax / majorXTickSpacing); i++) {
					g.drawLine((int) (i * majorXTickSpacing * xScale + xOffset), (int) (innerHeight + topInset),
							(int) (i * majorXTickSpacing * xScale + xOffset), (int) (innerHeight + topInset + majorTickLength));
					if (font != null) {
				...
				for (int i = (int) Math.ceil(xMin / minorXTickSpacing); i <= (int) Math.floor(xMax / minorXTickSpacing); i++) {
					g.drawLine((int) (i * minorXTickSpacing * xScale + xOffset), (int) (innerHeight + topInset),
							(int) (i * minorXTickSpacing * xScale + xOffset), (int) (innerHeight + topInset + minorTickLength));
				}
				g.setColor(tickColor);
				for (int i = (int) Math.ceil(xMin / minorXTickSpacing); i <= (int) Math.floor(xMax / minorXTickSpacing); i++) {
					g.drawLine((int) (i * minorXTickSpacing * xScale + xOffset), (int) (innerHeight + topInset),
				...
				for (int i = (int) Math.floor(Math.log(xMin) / Math.log(10.0)); i <= (int) Math
					...
					g.drawLine((int) (i * xScale + xOffset), (int) (innerHeight + topInset), (int) (i * xScale + xOffset),
	...
	protected void calcInsets() {
		...
		if (bgImage != null) {
//			img = bgImage.getScaledInstance(innerWidth, innerHeight, Image.SCALE_FAST);
		}
	}
