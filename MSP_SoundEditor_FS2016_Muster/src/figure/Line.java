	...
	protected double minDistance(double x, double y, double tolerance) {
		...
		for (int i = 0; i < xDataInt.length - 1; i++) {
			...
			if (Point2D.distance(xDataInt[i], yDataInt[i], x, y) < dPoints + tolerance
				...
				AffineTransform at2 = new AffineTransform(Math.cos(theta), Math.sin(theta), -Math.sin(theta),
		...
		if (!mouseEvent.isConsumed()) {
			...
			if (isSelected) {
				...
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).lineDragged((x - xOffset) / xScale,
							(-y - yOffset) / yScale, xData, yData);
				}
				parent.repaint();
			}
		}
	...
	public void mousePressed(MouseEvent mouseEvent) {
		...
		if (!mouseEvent.isConsumed()) {
			...
			if (minDistance(x, y, 5.0) < 5.0) {
				...
				isSelected = true;
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).linePressed((x - xOffset) / xScale,
							(-y - yOffset) / yScale, xData, yData);
				}
				parent.repaint();
			}
	...
	public void mouseReleased(MouseEvent mouseEvent) {
		...
		if (!mouseEvent.isConsumed()) {
			...
			if (isSelected) {
				...
				isSelected = false;
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).lineReleased((x - xOffset) / xScale,
							(y - yOffset) / yScale, xData, yData);
				}
				parent.repaint();
			}
