...
public class PlotAxes extends AbstractAxes implements MouseListener, MouseMotionListener {
	...
	protected Font font = null; //new Font("TimesRoman", Font.BOLD, 12);
	...
	public void paintComponent(Graphics z) {
		...
		if (tickColor != null) {
			...
			if (this.axesMode == LIN_LIN) {
				for (int i = (int) Math.ceil(xMin / majorXTickSpacing); i <= (int) Math.floor(xMax / majorXTickSpacing); i++) {
					...
					if (font != null) {
						String tickText = "" + Math.round(i * majorXTickSpacing);
			...
			for (int i = (int) Math.ceil(yMin / majorYTickSpacing); i <= (int) Math.floor(yMax / majorYTickSpacing); i++) {
				...
				if (font != null) {
					String tickText = "" + Math.round(i * majorYTickSpacing * 10) / 10.0;
		...
		if (txtColor != null) {
			...
			if (title != null) {
				...
				g.drawString(title, (int) ((innerWidth - r.getWidth()) / 2 + leftInset), (int) ((topInset + r.getHeight()) / 2));
			}
			// x-Label
			if (xLabel != null) {
		...
		if (gridColor != null) {
			...
			if (this.axesMode == LIN_LIN) {
				...
				for (int i = (int) Math.ceil(Math.log(xMin) / Math.log(10.0)); i <= (int) Math.floor(Math.log(xMax)
						/ Math.log(10.0)); i++) {
