...
public class DPIFixV3 {
	...
	private static final float dpi = 1.0f * (float) Math.sqrt(screen.getWidth() * screen.height + screen.width * screen.width)
			/ diag;
	public static final float fontScaler = 1 * fontSize * dpi / 1200f;

	public static void scaleSwingFonts() {
		scaleSwingFonts(fontScaler);
	}
...
//Font newFont = new FontUIResource(oldFont.getFontName(), oldFont.getStyle(), (int)(oldFont.getSize2D() * fontScaler));
//Font newFont = oldFont.deriveFont((float) (oldFont.getSize2D() * fontScaler));
