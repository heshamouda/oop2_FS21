...
/**
 ...
 $Source: $<br>
 */
public class Line extends AxesObject {
	...
	protected Vector markerVector = new Vector();
	protected boolean isSelected = false;
	protected Vector lineListenerVector = new Vector();
	...
	public void mousePressed(MouseEvent mouseEvent) {
		...
		if (markerVector.size() != 0) {
			for (int i = markerVector.size() - 1; i >= 0 && !mouseEvent.isConsumed(); i--) {
				((Marker) markerVector.elementAt(i)).mousePressed(mouseEvent);
			}
		}
		if (!mouseEvent.isConsumed()) {
			...
			if (minDistance(x, y, 5.0) < 5.0) {
				...
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).linePressed((x - xOffset) / xScale, (-y - yOffset)
							/ yScale, xData, yData);
				}
				parent.repaint();
			}
	...
	public void mouseDragged(MouseEvent mouseEvent) {
		...
		if (markerVector.size() != 0) {
			for (int i = markerVector.size() - 1; i >= 0 && !mouseEvent.isConsumed(); i--) {
				((Marker) markerVector.elementAt(i)).mouseDragged(mouseEvent);
			}
		}
		if (!mouseEvent.isConsumed()) {
			if (lineListenerVector.size() == 0) {
			...
			if (isSelected) {
				...
				int y = mouseEvent.getY();
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).lineDragged((x - xOffset) / xScale, (-y - yOffset)
							/ yScale, xData, yData);
				}
				parent.repaint();
			}
	...
	public void mouseReleased(MouseEvent mouseEvent) {
		...
		if (markerVector.size() != 0) {
			for (int i = markerVector.size() - 1; i >= 0 && !mouseEvent.isConsumed(); i--) {
				((Marker) markerVector.elementAt(i)).mouseReleased(mouseEvent);
			}
		}
		if (!mouseEvent.isConsumed()) {
			if (lineListenerVector.size() == 0) {
			...
			if (isSelected) {
				...
				isSelected = false;
				for (int i = 0; i < lineListenerVector.size(); i++) {
					((LineListener) lineListenerVector.elementAt(i)).lineReleased((x - xOffset) / xScale, (y - yOffset)
							/ yScale, xData, yData);
				}
				parent.repaint();
			}
	...
	protected double minDistance(double x, double y, double tolerance) {
		...
		for (int i = 0; i < xDataInt.length - 1; i++) {
			...
			if (Point2D.distance(xDataInt[i], yDataInt[i], x, y) < dPoints + tolerance
					|| Point2D.distance(xDataInt[i + 1], yDataInt[i + 1], x, y) < dPoints + tolerance) {
				...
				AffineTransform at2 = new AffineTransform(Math.cos(theta), Math.sin(theta), -Math.sin(theta), Math.cos(theta),
						0.0, 0.0);
				at2.concatenate(at1);
